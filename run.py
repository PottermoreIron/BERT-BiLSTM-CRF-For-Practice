...
def run():
    ...
    # build dataset
    # dataset一个元素有三个数组，分别是token2id数组，一个position数组和一个label数组。
    ...
    # build data_loader
    # 一个元素是一个batch
    # 一个batch里包含多条padding后的token2id数组(用0padding)，多条labelStart数组(其实就是用1标注word所在位置的数组)，和多条padding后的label数组(用-1padding)
